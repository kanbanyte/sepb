...
import onrobot_rg_modbus_tcp.rg2_modbus
from onrobot_rg_control.rg2_base import OnRobotBaseRG
...
class OnRobotRGTcpNode(OnRobotBaseRG):
	def __init__(self):
		...
		self.ip = self.declare_parameter('/onrobot/ip', '172.21.0.121').get_parameter_value().string_value
		self.port = self.declare_parameter('/onrobot/port', '54321').get_parameter_value().string_value
		...
		self.client = onrobot_rg_modbus_tcp.rg2_modbus.communication(self.dummy)
		self.prev_msg = []
		...
		self.rst_srv = self.create_service(Trigger, "/onrobot_rg/restart_power", self.restartPowerCycle)
		...
		self.sub = self.create_subscription(OnRobotRGOutput, 'OnRobotRGOutput', self.refreshCommand, 10)
	...
	def restartPowerCycle(self, request):
		...
		# rospy.sleep(1)
		# TODO: implement
		return Trigger.Response(success=None, message=None)
	...
	def timer_callback(self):
		...
		# not busy
		if not int(format(status.g_sta, '016b')[-1]):
			# find new message
			if not self.prev_msg == self.message:
