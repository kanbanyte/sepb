...
class OnRobotBaseRG(Node):
	"""Base class (communication protocol agnostic) for sending commands and receiving the status of the OnRobot RG gripper."""
	...
	def verifyCommand(self, command):
		...
		if self.gtype == 'rg2':
			...
			self.get_logger().error(self.get_name() + ": Select the gripper type from rg2 or rg6.")
			rclpy.shutdown()
		...
		if command.r_ctr not in [1, 8, 16]:
			self.get_logger(self.get_name() + ": Select the mode number from" + "1 (grip), 8 (stop), or 16 (grip_w_offset).")

		# Return the modified command
		return command

	def refreshCommand(self, command):
		"""Updates the command which will be sent during the next sendCommand() call."""
	...
	def getStatus(self):
		"""Requests the status from the gripper and return it in the OnRobotRGInput msg type."""
